...
let testLanguagePlugin() =
    printfn "Deprecated: calling `make test-language-plugin` instead"
    let cmd = Cli.Wrap("make").WithArguments("test-language-plugin").WithWorkingDirectory(repositoryRoot)
    let output = cmd.ExecuteAsync().GetAwaiter().GetResult()
    if output.ExitCode <> 0 then
        failwith "Testing pulumi-language-dotnet failed"
