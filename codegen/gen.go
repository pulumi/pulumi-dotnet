...
func (mod *modContext) genFunction(w io.Writer, fun *schema.Function) error {
	...
	if fun.ReturnType != nil {
		if objectType, ok := fun.ReturnType.(*schema.ObjectType); ok && objectType != nil {
			// for object return types, assume a Result type is generated
			// and reference it from here
			typeParameter = fmt.Sprintf("%sResult", className)
		} else {
			typeParameter = mod.typeString(fun.ReturnType, "", false, false, true)
		}
	}
	...
	if !fun.MultiArgumentInputs {
		...
		// new line and indent
		fmt.Fprint(w, "            ")
		fmt.Fprintf(w, "=> global::Pulumi.Deployment.Instance.InvokeAsync%s", typeParamOrEmpty(typeParameter))
		...
		fmt.Fprintf(w, "InvokeAsync%s", typeParamOrEmpty(typeParameter))
	...
	case *schema.Function:
		...
		if member.ReturnType != nil {
			if objectType, ok := member.ReturnType.(*schema.ObjectType); ok && objectType != nil {
				mod.getTypeImports(objectType, false, imports, seen)
			}
		}
		return
