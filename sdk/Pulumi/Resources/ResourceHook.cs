...
namespace Pulumi
{
    ...
    /// </para>
    /// <para>
    /// By default, an exception from a <c>Before*</c> hook causes the resource operation to fail. An exception from an
    /// <c>After*</c> hook fails the deployment. The resource operation itself has already succeeded, so its result is
    /// recorded in state. Set <see cref="ResourceHookOptions.IgnoreErrors"/> on the hook to log a warning instead.
    /// </para>
    /// <para>
